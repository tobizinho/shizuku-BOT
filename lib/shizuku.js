...
module.exports = sock = async(sock, mek, settings, color) => {
 try {
  ...
  switch (command) {
   ...
   case 'ban':
    var message = body.slice(5);
    participants = message.replace(/[- +@)(]/g, '') + "@s.whatsapp.net" === "@s.whatsapp.net" ? mek.message[type].contextInfo.participant: message.replace(/[- +@)(]/g, '') + "@s.whatsapp.net";
    sock.groupParticipantsUpdate(from, [participants], "remove");
    break;
    
    case 'resen':
     var message = body.slice(7);
     reply(JSON.stringify(message, null, 2));
     break;
   ...
   case 'play':
    ...
    fetchJson(`https://ayanami-apis.herokuapp.com/api/download/playmp3?nome=${teks}&apikey=apikeyFree30days`).then(({
     ...
     if (resultado.messageError?.statusCode !== 410) {
      ...
      reply(`❌ Nenhum áudio/música encontrado com o título: ${teks} 💢`);
     }
    },
     (err) => {
   ...
   default:
    ...
    if (budy.includes('>>')) {
     ...
     if (!isOwner) return;
     try {
      reply(JSON.stringify((eval(message)), null, 2));
     } catch (e) {
      err = String(e);
      reply(err);
     };
